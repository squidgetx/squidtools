...
def generate_chat_response(messages, model="gpt-3.5-turbo", attempt=0, smart_context_setting=True):
    try:
        response = client.chat.completions.create(model=model,
        temperature=0.5,
        messages=messages)
        return response
    except (error.ServiceUnavailableError, error.Timeout) as e:
        attempt += 1
